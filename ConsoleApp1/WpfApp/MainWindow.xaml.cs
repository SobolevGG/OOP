...
namespace HydroGeneratorOptimization
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            initialFlowRatesItemsControl.ItemsSource = Enumerable.Range(1, 12).Select(i => new Tuple<string, double>($"Гидрогенератор {i}", 0.0));
        }
        ...
        private void RunOptimization_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Явное приведение типа для элементов в double
                double[] initialFlowRates = initialFlowRatesItemsControl.Items.OfType<Tuple<string, double>>().Select(item => item.Item2).ToArray();
        ...
        private void RunPowerCalculation_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Здесь добавьте код для обработки расчёта мощности по расходам
                // Используйте значения из TextBox'ов (flowRate1TextBox, flowRate2TextBox и т. д.)
                // Выведите результаты в соответствующие элементы для отображения результатов
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка при выполнении расчёта мощности: {ex.Message}", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
