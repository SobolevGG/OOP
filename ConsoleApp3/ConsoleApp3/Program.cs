...
class Program
{
    static void Main()
    {
        CreateListAndSaveToXML();

        // Чтение и вывод содержимого XML-документа
        List<GeneratorFlow> readGeneratorFlows = ReadXmlDocument("generatorFlows.xml");
        Console.WriteLine("Содержимое XML-документа:");

        foreach (var generatorFlow in readGeneratorFlows)
        {
            Console.WriteLine($"Генератор: {generatorFlow.Name}, Расход воды: {generatorFlow.WaterFlow}");
        }

        // Проверка количества гидрогенераторов
        CheckXmlDocument(readGeneratorFlows, 12);
    }
}
